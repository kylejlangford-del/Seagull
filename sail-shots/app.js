(() => {
  ...
  // the gallery. This panel replaces that back-and-forth: a floating
  // control lets him nudge the offset himself (plus/minus 1s / 0.5s
  // buttons, or type one in directly) and see every Sept 10 photo's
  // displayed time and boat data update together, live, before deciding to
  // publish it.
  //
  // Data comes from sail-shots/sync-table.json, a precomputed lookup table
  // (all 23 boat-data columns, at every 0.5s offset from -2.0s to +8.0s,
  // for all 115 Sept 10 shots) built once on the boat-data CSV so this
  // panel never needs to touch the CSV itself or recompute anything client
  ...
  let syncPanelErrorMessage = '';

  const el2 = {}; // small second element bag for the panel, kept separate from el{} above (which is bound to fixed ids already in index.html)

  function injectSyncPanelStyles() {
    if (document.getElementById('syncPanelStyles')) return;
    const style = document.createElement('style');
    style.id = 'syncPanelStyles';
    style.textContent = `
      #syncPanel { position: fixed; right: 16px; bottom: 16px; z-index: 5000; width: 258px;
        background: #1b1f24; color: #e8ebef; border: 1px solid #3a4048; border-radius: 10px;
        box-shadow: 0 6px 24px rgba(0,0,0,0.4); font: 13px/1.4 -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
        padding: 12px; }
      #syncPanel h4 { margin: 0 0 6px; font-size: 12.5px; font-weight: 600; letter-spacing: 0.2px; color: #cfd6de; }
      #syncPanel .syncPanel__offset { font-size: 20px; font-weight: 700; text-align: center; margin: 6px 0; color: #7fd0ff; }
      #syncPanel .syncPanel__status { text-align: center; font-size: 11.5px; margin-bottom: 8px; color: #9aa4af; }
      #syncPanel .syncPanel__status--dirty { color: #ffb454; }
      #syncPanel .syncPanel__status--saved { color: #6cd67a; }
      #syncPanel .syncPanel__status--error { color: #ff6b6b; }
      #syncPanel .syncPanel__row { display: flex; gap: 6px; margin-bottom: 8px; }
      #syncPanel button.syncPanel__step { flex: 1; background: #2a3038; color: #e8ebef; border: 1px solid #454c55;
        border-radius: 6px; padding: 6px 4px; font-size: 12.5px; cursor: pointer; }
      #syncPanel button.syncPanel__step:hover { background: #333b45; }
      #syncPanel button.syncPanel__step:disabled { opacity: 0.4; cursor: default; }
      #syncPanel .syncPanel__manualRow { display: flex; gap: 6px; margin-bottom: 8px; align-items: center; }
      #syncPanel input.syncPanel__manualInput { width: 72px; background: #101316; color: #e8ebef;
        border: 1px solid #454c55; border-radius: 6px; padding: 5px 6px; font-size: 12.5px; }
      #syncPanel button.syncPanel__go { background: #2a3038; color: #e8ebef; border: 1px solid #454c55;
        border-radius: 6px; padding: 5px 10px; font-size: 12.5px; cursor: pointer; }
      #syncPanel button.syncPanel__publish { width: 100%; background: #1f6f3c; color: #fff; border: none;
        border-radius: 6px; padding: 8px; font-size: 13px; font-weight: 600; cursor: pointer; margin-bottom: 6px; }
      #syncPanel button.syncPanel__publish:disabled { background: #2a3038; color: #77808a; cursor: default; }
      #syncPanel button.syncPanel__reset { width: 100%; background: transparent; color: #9aa4af; border: 1px solid #3a4048;
        border-radius: 6px; padding: 6px; font-size: 12px; cursor: pointer; }
      #syncPanel button.syncPanel__collapse { position: absolute; top: 8px; right: 10px; background: none; border: none;
        color: #9aa4af; font-size: 14px; cursor: pointer; padding: 0; line-height: 1; }
      #syncPanel.is-collapsed .syncPanel__body { display: none; }
    `;
    document.head.appendChild(style);
  }

  function buildSyncPanelDom() {
    ...
    panel.innerHTML = `
      ...
      <div class="syncPanel__body" id="syncPanelMain">
        ...
        <div class="syncPanel__row">
          <button type="button" class="syncPanel__step" data-delta="-1">-1s</button>
          <button type="button" class="syncPanel__step" data-delta="-0.5">-0.5s</button>
          <button type="button" class="syncPanel__step" data-delta="0.5">+0.5s</button>
          <button type="button" class="syncPanel__step" data-delta="1">+1s</button>
        </div>
        <div class="syncPanel__manualRow">
          <input type="number" step="0.5" class="syncPanel__manualInput" id="syncManualInput" placeholder="e.g. 4.0" />
          <button type="button" class="syncPanel__go" id="syncManualGo">Set</button>
        </div>
    ...
    el2.manualGo = panel.querySelector('#syncManualGo');
    el2.publishBtn = panel.querySelector('#syncPublishBtn');
    el2.resetBtn = panel.querySelector('#syncResetBtn');
    el2.collapseBtn = panel.querySelector('.syncPanel__collapse');

    panel.querySelectorAll('.syncPanel__step').forEach(btn => {
      btn.addEventListener('click', () => {
        const delta = Number(btn.dataset.delta);
        applySyncOffset(syncOffsetSeconds + delta);
      });
    });
    el2.manualGo.addEventListener('click', () => {
      const val = Number(el2.manualInput.value);
      if (!Number.isNaN(val)) applySyncOffset(val);
    });
    el2.manualInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        const val = Number(el2.manualInput.value);
        if (!Number.isNaN(val)) applySyncOffset(val);
      }
    });
    el2.publishBtn.addEventListener('click', () => publishSyncOffset());
    el2.resetBtn.addEventListener('click', () => resetSyncOffset());
    el2.collapseBtn.addEventListener('click', () => {
      panel.classList.toggle('is-collapsed');
      el2.collapseBtn.innerHTML = panel.classList.contains('is-collapsed') ? '&plus;' : '&minus;';
    });
  }
  ...
  function setSyncStatus(kind, message) {
    if (!el2.statusText) return;
    el2.statusText.classList.remove('syncPanel__status--dirty', 'syncPanel__status--saved', 'syncPanel__status--error');
    if (kind === 'dirty') {
      el2.statusText.classList.add('syncPanel__status--dirty');
      el2.statusText.textContent = 'Preview -- not published yet';
    } else if (kind === 'saved') {
      el2.statusText.classList.add('syncPanel__status--saved');
      el2.statusText.textContent = 'Published -- live in about a minute';
    } else if (kind === 'saving') {
      el2.statusText.textContent = 'Publishing...';
    } else if (kind === 'error') {
      el2.statusText.classList.add('syncPanel__status--error');
      el2.statusText.textContent = message || 'Something went wrong';
    } else if (kind === 'matches-published') {
      el2.statusText.textContent = "Matches what's published";
    } else {
      el2.statusText.textContent = message || '';
    }
  }

  function updateSyncPanelDisplay() {
    if (!el2.offsetVal) return;
    const sign = syncOffsetSeconds > 0 ? '+' : '';
    el2.offsetVal.textContent = `${sign}${syncOffsetSeconds.toFixed(1)}s`;
    el2.manualInput.value = syncOffsetSeconds.toFixed(1);
    el2.publishBtn.disabled = syncPublished;
    if (syncPublished) setSyncStatus('matches-published');
  ...
  async function publishSyncOffset() {
    if (!getGithubToken()) {
      alert("Connect GitHub first (top of the page) -- the sync offset needs a token on file to publish.");
      return;
    }
    const sign = syncOffsetSeconds > 0 ? '+' : '';
    ...
    if (ok) {
      syncPublished = true;
      setSyncStatus('saved');
      setTimeout(() => updateSyncPanelDisplay(), 2500);
    } else {
      el2.publishBtn.disabled = false;
      setSyncStatus('error', 'Publish failed -- see the status bar above for retry');
    }
  }

  // Reloads the manifest as it's actually published (discarding any
  // preview offset that hasn't been published), then re-detects the offset
  // that implies.
  ...
  async function initSyncPanel() {
    ...
    try {
      ...
      setSyncStatus('error', syncPanelErrorMessage);
      console.error('Sync panel: failed to load sync-table.json', err);
    }
  }
